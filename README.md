...
All commands below should be run in `./typescript`.

1. Install dependencies with `npm install`.
2. Start the local stack with `npm start`.
3. Check the API health endpoint with `GET http://localhost:3000/api/healthcheck`.
4. Check the bundled external system with `GET http://localhost:5000/students?class=2&offset=1&limit=2`.
5. Use `npm run start:dev` if you want watch mode during development.
...
The default schema lives in `typescript/database/DDL.sql`.
`docker-compose.yml` mounts `./database` into `/docker-entrypoint-initdb.d`, so the schema is created automatically the first time MySQL starts.
If you choose a different initialization method, add the manual steps here so the database can still be created locally.
...
- The external system is included only as a dependency for question 2.
- The external check endpoint uses `GET`, not `POST`.
